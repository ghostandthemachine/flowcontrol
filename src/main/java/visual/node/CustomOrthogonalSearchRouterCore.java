...
import visual.node.CustomOrthogonalSearchRouter.*;
import visual.node.CustomOrthogonalSearchRouterRegion.*;

/**
 * @author Jon Rose
 */
final class CustomOrthogonalSearchRouterCore {
    ...
    private final CustomOrthogonalSearchRouterRegion[] regions = new CustomOrthogonalSearchRouterRegion[MAXIMAL_DEPTH  + 1];

    private Point[] bestControlPoints;
    private int bestControlPointsPrice;

    public CustomOrthogonalSearchRouterCore (Scene scene, ArrayList<Rectangle> verticalCollisions, ArrayList<Rectangle> horizontalCollisions, Point sourcePoint, Anchor.Direction sourceDirection, Point targetPoint, Anchor.Direction targetDirection) {
        this.scene = scene;
    ...
    public CustomOrthogonalSearchRouter.Solution route () {
        ...
        CustomOrthogonalSearchRouterRegion region =
                new CustomOrthogonalSearchRouterRegion(sourceBoundaryPoint.x, sourceBoundaryPoint.y,
        ...
        Solution solution =
                new CustomOrthogonalSearchRouter.Solution(bestControlPointsPrice, Arrays.asList(bestControlPoints)) ;
    ...
    private void search (CustomOrthogonalSearchRouterRegion region) {
        Rectangle inter = region.intersection (scene.getMaximumBounds ());
        region = new CustomOrthogonalSearchRouterRegion (inter.x, inter.y, inter.width, inter.height, region.getDirection (), region.getDepth ());

        if (region.width < 0 || region.height < 0)
            return;
        assert region.x >= CustomOrthogonalSearchRouterRegion.MIN_INT_REGION;
        assert region.y >= CustomOrthogonalSearchRouterRegion.MIN_INT_REGION;
        assert region.x + region.width <= CustomOrthogonalSearchRouterRegion.MAX_INT_REGION;
        assert region.y + region.height <= CustomOrthogonalSearchRouterRegion.MAX_INT_REGION;
//        System.out.println ("REG: " + region);
        ...
        ArrayList<CustomOrthogonalSearchRouterRegion> subRegions = region.parseSubRegions (collisions);
...
//            System.out.println ("FOUND");
            ...
            if (! subRegions.isEmpty ()) {
                for (CustomOrthogonalSearchRouterRegion subRegion : subRegions) {
                    search (subRegion);
                }
            }
        }
    ...
    private void constructControlPoints (int depth) {
        ...
        if (desiredDirection != regions[depth].getDirection ()) {
            final CustomOrthogonalSearchRouterRegion region = regions[depth];
            depth++;
            regions[depth] = new CustomOrthogonalSearchRouterRegion (region.x, region.y, region.width, region.height, desiredDirection, depth);
        }
        ...
        for (int a = 0; a < depth; a ++) {
            final CustomOrthogonalSearchRouterRegion region = regions[a];
            ...
            if (region.isHorizontal ()) {
                ...
                if (a > 0) {
                    ...
                    if (y1 <= CustomOrthogonalSearchRouterRegion.MIN_INT_REGION && y2 < CustomOrthogonalSearchRouterRegion.MAX_INT_REGION)
                        yy = y2 - CustomOrthogonalSearchRouter.SPACING_EDGE;
                    else if (y1 > CustomOrthogonalSearchRouterRegion.MIN_INT_REGION && y2 >= CustomOrthogonalSearchRouterRegion.MAX_INT_REGION)
                        yy = y1 + CustomOrthogonalSearchRouter.SPACING_EDGE;
                    else
                ...
                if (a > 0) {
                    ...
                    if (x1 <= CustomOrthogonalSearchRouterRegion.MIN_INT_REGION && x2 < CustomOrthogonalSearchRouterRegion.MAX_INT_REGION)
                        xx = x2 - CustomOrthogonalSearchRouter.SPACING_EDGE;
                    else if (x1 > CustomOrthogonalSearchRouterRegion.MIN_INT_REGION && x2 >= CustomOrthogonalSearchRouterRegion.MAX_INT_REGION)
                        xx = x1 + CustomOrthogonalSearchRouter.SPACING_EDGE;
                    else
